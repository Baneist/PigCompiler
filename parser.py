...
def findClosure(parent:list)->list:
    ...
    while top < len(closure_set):
        ...
        if pos < len(nowc['right']) and not isTerminalSymbol(nowc['right'][pos]):#如果是非终结符，进行拓展
            follow_symbol = nowc['right'][pos+1:] if pos < len(nowc) - 1 else []#后面用于求first集的子串
...
def initProjectSet():
    ...
    while top < len(project_set):
        for x in symbol_list:
            ...
            #if len(next_set) > 0:#构造action和goto
            ...
            if len(next_set) > 0:
                if not isTerminalSymbol(x):
                    ...
                    if len(next_set) == 1 and next_set[0]['dot'] == len(next_set[0]['right']): #处理规约项目
                        ns = {'left':next_set[0]['left'], 'right':next_set[0]['right']}
                        action_goto[(top, x)] = ['r', productions.index(ns)] if ns['left'] != '<开始>' else ['acc']
                    else: #处理移进项目
